...
var PseudoClassSelector = function PseudoClassSelector() {
    ...
    if (this.scanner.tokenType === LEFTPARENTHESIS$5) {
        ...
        if (this.pseudo.hasOwnProperty(nameLowerCase)) {
            ...
            children = new list().appendData(this.Raw(BALANCED$2, 0, 0));
        }
...
var PseudoElementSelector = function PseudoElementSelector() {
    ...
    if (this.scanner.tokenType === LEFTPARENTHESIS$6) {
        ...
        if (this.pseudo.hasOwnProperty(nameLowerCase)) {
            ...
            children = new list().appendData(this.Raw(BALANCED$3, 0, 0));
        }
        ...
        if (node.type === 'ExportDefaultDeclaration') {
            ...
            node.declaration.properties.forEach(function (prop) {
                ...
                if (propValidator) {
                    ...
                    if (match) {
                        validator.error("Unexpected property '" + prop.key
                            .name + "' (did you mean '" + match + "'?)", prop.start);
                    }
                    else if (/FunctionExpression/.test(prop.value.type)) {
                        validator.error("Unexpected property '" + prop.key
                            .name + "' (did you mean to include it in 'methods'?)", prop.start);
                    }
                    else {
...
function validateEventHandlerCallee(validator, attribute, refCallees) {
    ...
    if ((callee.type === 'Identifier' && validBuiltins.has(callee.name)) ||
        ...
        return;
    var validCallees = ['this.*', 'event.*'].concat(Array.from(validBuiltins), Array.from(validator.methods.keys()));
...
function validateElement(validator, node, refs, refCallees) {
...
function validateHtml(validator, html$$1) {
    ...
    function visit(node) {
        if (node.type === 'Element') {
            if (elementDepth === 0 &&
                validator.namespace !== svg &&
                svg$1.test(node.name)) {
...
function visitEachBlock(generator, block, state, node, elementStack, componentStack) {
    var each_block = generator.getUniqueName("each_block");
    var create_each_block = node._block.name;
    var each_block_value = node._block.listName;
    var iterations = block.getUniqueName(each_block + "_iterations");
    var params = block.params.join(', ');
    var anchor = node.needsAnchor
    ...
    var length = "[\u2702" + c + "-" + (c + 4) + "\u2702]";
    var mountOrIntro = node._block.hasIntroMethod ? 'intro' : 'mount';
    var vars = {
    ...
    if (node["else"]) {
        var each_block_else = generator.getUniqueName(each_block + "_else");
        ...
        block.builders.mount.addBlock((_b = ["\n\t\t\tif ( ", " ) {\n\t\t\t\t", ".", "( ", ", null );\n\t\t\t}\n\t\t"], _b.raw = ["\n\t\t\tif ( ", " ) {\n\t\t\t\t", ".", "( ",
            ", null );\n\t\t\t}\n\t\t"], deindent(_b, each_block_else, each_block_else, mountOrIntro, state.parentNode ||
            '#target')));
        ...
        block.builders.unmount.addLine("if ( " + each_block_else + " ) " + each_block_else + ".unmount()");
...
function keyed(generator, block, state, node, snippet, _a) {
    var each_block = _a.each_block, create_each_block = _a.create_each_block, each_block_value = _a.each_block_value, length = _a.length, params = _a.params, anchor = _a.anchor, mountOrIntro = _a.mountOrIntro;
    var key = block.getUniqueName('key');
    var lookup = block.getUniqueName(each_block + "_lookup");
    var iteration = block.getUniqueName(each_block + "_iteration");
    var head = block.getUniqueName(each_block + "_head");
    var last = block.getUniqueName(each_block + "_last");
    var expected = block.getUniqueName(each_block + "_expected");
    ...
    if (node._block.hasOutroMethod) {
        var fn = block.getUniqueName(each_block + "_outro");
        ...
        var fn = block.getUniqueName(each_block + "_destroy");
...
function visitSlot(generator, block, state, node, elementStack, componentStack) {
    ...
    block.addElement(name, "@createElement('slot')", "@claimElement(" + state.parentNodes + ", 'slot', {" + (slotName !== 'default' ? " name: '" + slotName + "' " : '') + "})", state.parentNode);
    if (generator.hydratable) {
        block.builders.claim.addLine("var " + node._state.parentNodes + " = @children(" + name + ");");
    }
    if (slotName !== 'default') {
        block.builders.hydrate.addBlock((_a = ["\n\t\t\t@setAttribute(", ", 'name', '", "');\n\t\t"], _a.raw = ["\n\t\t\t@setAttribute(", ", 'name', '", "');\n\t\t"], deindent(_a, name, slotName)));
    }
    block.builders.mount.addLine("#component.slots." + slotName + " = " + name + ";");
    block.builders.unmount.addLine("#component.slots." + slotName + " = null;");
    ...
    node.children.forEach(function (child) {
        visit$1(generator, block, node._state, child, elementStack.concat(node), componentStack);
    });
    ...
    block.builders.mount.addBlock((_b = ["\n\t\tif (", ") {\n\t\t\t@appendNode(", ", ", ");\n\t\t}\n\t"], _b.raw = ["\n\t\tif (", ") {\n\t\t\t@appendNode(", ", ", ");\n\t\t}\n\t"], deindent(_b, content_name, content_name, name)));
    ...
    block.builders.unmount.addBlock((_c = ["\n\t\tif (", ") {\n\t\t\twhile (", ".firstChild) @appendNode(", ".firstChild, ", ");\n\t\t}\n\t"], _c.raw = ["\n\t\tif (", ") {\n\t\t\twhile (", ".firstChild) @appendNode(", ".firstChild, ", ");\n\t\t}\n\t"], deindent(_c, content_name, name, name, content_name)));
    var _a, _b, _c;
}
...
function visitComponent(generator, block, state, node, elementStack, componentStack) {
    ...
    block.builders.mount.addLine(name + "._fragment.mount( " + (state.parentNode || '#target') + ", " + (state.parentNode ? 'null' : 'anchor') + " );");
    if (!state.parentNode)
...
function visitWindow(generator, block, node) {
    ...
    if (bindings.scrollX && bindings.scrollY) {
        ...
        block.builders.init.addBlock((_b = ["\n\t\t\t#component.observe( '", "', function ( ", " ) {\n\t\t\t\tif ( ", " ) return;\n\t\t\t\twindow.scrollTo( ", " );\n\t\t\t});\n\t\t"], _b.raw = ["\n\t\t\t#component.observe( '",
            "', function ( ", " ) {\n\t\t\t\tif ( ", " ) return;\n\t\t\t\twindow.scrollTo( ", " );\n\t\t\t});\n\t\t"], deindent(_b, bindings.scrollX ||
            bindings.scrollY, isX ? 'x' : 'y', lock, isX ? 'x, window.scrollY' : 'window.scrollX, y')));
    }
...
function visitBinding(generator, block, state, node, attribute) {
    ...
    var eventName = getBindingEventName(node, attribute);
    var handler = block.getUniqueName(state.parentNode + "_" + eventName + "_handler");
    var isMultipleSelect = node.name === 'select' &&
        node.attributes.find(function (attr) { return attr.name.toLowerCase() === 'multiple'; }); // TODO use getStaticAttributeValue
    ...
    var bindingGroup = attribute.name === 'group'
        ...
        : null;
    var value = getBindingValue(generator, block, state, node, attribute, isMultipleSelect, bindingGroup, type);
    ...
    block.builders.init.addBlock((_g = ["\n\t\tfunction ", " () {\n\t\t\t", " = true;\n\t\t\t", "\n\t\t\t", " = false;\n\t\t}\n\t"], _g.raw = ["\n\t\tfunction ", " () {\n\t\t\t", " = true;\n\t\t\t", "\n\t\t\t", " = false;\n\t\t}\n\t"], deindent(_g, handler, lock, setter, lock)));
    ...
    if (updateCondition !== null) {
        ...
        block.builders.update.addBlock((_k = ["\n\t\t\tif ( ", " ) {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], _k.raw = ["\n\t\t\tif ( ", " ) {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], deindent(_k, updateCondition, updateElement)));
    }
    ...
    if (block.contexts.has(name)) {
        ...
        return (_a = ["\n\t\t\tvar list = ", "._svelte.", ";\n\t\t\tvar index = ", "._svelte.", ";\n\t\t\t", "\n\t\t\tlist[index]", " = ", ";\n\n\t\t\t", "\n\t\t"], _a.raw = ["\n\t\t\tvar list = ", "._svelte.", ";\n\t\t\tvar index = ", "._svelte.", ";\n\t\t\t", "\n\t\t\tlist[index]", " = ", ";\n\n\t\t\t",
            ...
            ? "#component.set({ " + dependencies
                .map(function (prop) { return prop + ": state." + prop; })
                .join(', ') + " });"
            : "#component.set({ " + dependencies
                .map(function (prop) { return prop + ": #component.get( '" + prop + "' )"; })
                .join(', ') + " });"));
    }
    if (attribute.value.type === 'MemberExpression') {
        return (_b = ["\n\t\t\tvar state = #component.get();\n\t\t\t", " = ", ";\n\t\t\t#component.set({ ", " });\n\t\t"], _b.raw = ["\n\t\t\tvar state = #component.get();\n\t\t\t", " = ", ";\n\t\t\t#component.set({ ",
            " });\n\t\t"], deindent(_b, snippet, value, dependencies
            .map(function (prop) { return prop + ": state." + prop; })
            .join(', ')));
    }
    ...
    if (intro === outro) {
        ...
        if (outro) {
            ...
            var snippet = outro.expression
                ...
                : '{}';
            var fn = "@template.transitions." + outro.name;
            // TODO hide elements that have outro'd (unless they belong to a still-outroing
            // group) prior to their removal from the DOM
            block.builders.outro.addBlock((_e = ["\n\t\t\t\t", " = @wrapTransition( #component, ", ", ", ", ", ", false, null );\n\t\t\t\t", ".run( false, function () {\n\t\t\t\t\t#component.fire( 'outro.end', { node: ", " });\n\t\t\t\t\tif ( --#outros === 0 ) #outrocallback();\n\t\t\t\t});\n\t\t\t"], _e.raw = ["\n\t\t\t\t", " = @wrapTransition( #component, ", ", ", ", ", ", false, null );\n\t\t\t\t", ".run( false, function () {\n\t\t\t\t\t#component.fire( 'outro.end', { node: ", " });\n\t\t\t\t\tif ( --#outros === 0 ) #outrocallback();\n\t\t\t\t});\n\t\t\t"], deindent(_e, outroName, state.name, fn, snippet, outroName, state.name)));
        }
    }
    var _a, _b, _c, _d, _e;
}
...
function visitElement(generator, block, state, node, elementStack, componentStack) {
    ...
    var parentNode = slot ?
        componentStack[componentStack.length - 1]._state.name + "._slotted." + slot.value[0].data :
        state.parentNode;
    ...
    if (node.initialUpdate) {
        ...
        var needsStateObject = node.initialUpdateNeedsStateObject && state.isYield;
        if (needsStateObject) {
...
function getClaimStatement(generator, namespace, nodes, node) {
    var attributes = node.attributes
        ...
        .map(function (attr) { return quoteProp(attr.name) + ": true"; })
        .join(', ');
...
function visitIfBlock(generator, block, state, node, elementStack, componentStack) {
    var name = generator.getUniqueName("if_block");
    var anchor = node.needsAnchor
    ...
    block.builders.claim.addLine("" + if_name + name + ".claim( " + state.parentNodes + " );");
    if (node.needsAnchor) {
        block.addElement(anchor, "@createComment()", "@createComment()", state.parentNode);
    }
    else if (node.next) {
...
function simple(generator, block, state, node, branch, dynamic, _a) {
    ...
    var isTopLevel = !state.parentNode;
    var mountOrIntro = branch.hasIntroMethod ? 'intro' : 'mount';
    var targetNode = state.parentNode || '#target';
    var anchorNode = state.parentNode ? 'null' : 'anchor';
...
function compound(generator, block, state, node, branches, dynamic, _a) {
    ...
    var isTopLevel = !state.parentNode;
    var mountOrIntro = branches[0].hasIntroMethod ? 'intro' : 'mount';
    var targetNode = state.parentNode || '#target';
    var anchorNode = state.parentNode ? 'null' : 'anchor';
...
function visitTag(generator, block, state, node, name, update) {
    ...
    var init = shouldCache ? value : snippet;
    if (shouldCache)
    ...
    if (dependencies.length || hasChangeableIndex) {
        ...
        var condition = shouldCache ?
            (dependencies.length ? "( " + changedCheck + " ) && " + updateCachedValue : updateCachedValue) :
            changedCheck;
        block.builders.update.addConditional(condition, update(shouldCache ? value : snippet));
    }
    return { init: init };
}
...
function visitMustacheTag(generator, block, state, node) {
    var name = node._state.name;
    var init = visitTag(generator, block, state, node, name, function (value) { return name + ".data = " + value + ";"; }).init;
    block.addElement(name, "@createText( " + init + " )", "@claimText( " + state.parentNodes + ", " + init + " )", state.parentNode);
}

function visitRawMustacheTag(generator, block, state, node) {
    var name = node._state.basename;
    var before = node._state.name;
    var after = block.getUniqueName(name + "_after");
    var init = visitTag(generator, block, state, node, name, function (value) {
        return (_a = ["\n\t\t\t@detachBetween( ", ", ", " );\n\t\t\t", ".insertAdjacentHTML( 'afterend', ", " );\n\t\t"], _a.raw = ["\n\t\t\t@detachBetween( ", ", ", " );\n\t\t\t", ".insertAdjacentHTML( 'afterend', ", " );\n\t\t"], deindent(_a, before, after, before, value));
        var _a;
    }).init;
    // we would have used comments here, but the `insertAdjacentHTML` api only
    // exists for `Element`s.
    block.addElement(before, "@createElement( 'noscript' )", "@createElement( 'noscript' )", state.parentNode);
    block.addElement(after, "@createElement( 'noscript' )", "@createElement( 'noscript' )", state.parentNode);
...
function visitText(generator, block, state, node) {
    if (!node._state.shouldCreate)
        return;
    block.addElement(node._state.name, "@createText( " + stringify(node.data) + " )", "@claimText( " + state.parentNodes + ", " + stringify(node.data) + " )", state.parentNode);
}

function visitYieldTag(generator, block, state) {
    var parentNode = state.parentNode || '#target';
    block.builders.mount.addLine("if ( #component._yield ) #component._yield.mount( " + parentNode + ", null );");
    block.builders.unmount.addLine("if ( #component._yield ) #component._yield.unmount();");
}
...
function getGlobals(imports, options) {
    ...
    return imports.map(function (x) {
        ...
        if (!name) {
            if (x.name.startsWith('__import')) {
                var error = new Error("Could not determine name for imported module '" + x.source
...
function getIntro(format, options, imports) {
    if (format === 'es')
        return '';
    if (format === 'amd')
        return getAmdIntro(options, imports);
    if (format === 'cjs')
        return getCjsIntro(options, imports);
    if (format === 'iife')
        return getIifeIntro(options, imports);
    if (format === 'umd')
        return getUmdIntro(options, imports);
    if (format === 'eval')
        return getEvalIntro(options, imports);
    throw new Error("Not implemented: " + format);
}
function getAmdIntro(options, imports) {
    var sourceString = imports.length
        ? "[ " + imports
            .map(function (declaration) { return "'" + removeExtension(declaration.source.value) + "'"; })
            .join(', ') + " ], "
        : '';
...
function getCjsIntro(options, imports) {
    var requireBlock = imports
        ...
        return "var " + declaration.name + " = require( '" + declaration.source.value + "' );";
    })
        .join('\n\n');
    if (requireBlock) {
        return "'use strict';\n\n" + requireBlock + "\n\n";
...
function getUmdIntro(options, imports) {
    ...
    var amdDeps = imports.length
        ? "[" + imports
            .map(function (declaration) { return "'" + removeExtension(declaration.source.value) + "'"; })
            .join(', ') + "], "
        : '';
    var cjsDeps = imports
        .map(function (declaration) { return "require('" + declaration.source.value + "')"; })
        .join(', ');
    var globalDeps = getGlobals(imports, options);
    return ((_a = ["\n\t\t(function ( global, factory ) {\n\t\t\ttypeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(", ") :\n\t\t\ttypeof define === 'function' && define.amd ? define(", "", "factory) :\n\t\t\t(global.", " = factory(", "));\n\t\t}(this, (function (", ") { 'use strict';"], _a.raw = ["\n\t\t(function ( global, factory ) {\n\t\t\ttypeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(", ") :\n\t\t\ttypeof define === 'function' && define.amd ? define(", "", "factory) :\n\t\t\t(global.", " = factory(", "));\n\t\t}(this, (function (", ") { 'use strict';"], deindent(_a, cjsDeps, amdId, amdDeps, options.name, globalDeps, paramString(imports))) + '\n\n');
    var _a;
}
...
var Generator = (function () {
    function Generator(parsed, source, name, stylesheet, options) {
    ...
    Generator.prototype.generate = function (result, options, _a) {
        ...
        if (this.imports.length) {
            ...
            this.imports.forEach(function (declaration, i) {
                if (format === 'es') {
                    statements_1.push(_this.source.slice(declaration.start, declaration.end));
                    return;
                }
                ...
                namedImports.forEach(function (specifier) {
                    statements_1.push("var " + specifier.local.name + " = " + name + "." + specifier.imported.name);
                });
                if (defaultImport) {
                    statements_1.push(name + " = ( " + name + " && " + name + ".__esModule ) ? " + name + "['default'] : " + name + ";");
                }
            });
            result = statements_1.join('\n') + "\n\n" + result;
        }
        var pattern = /\[✂(\d+)-(\d+)$/;
        ...
        var _b = this.stylesheet.render(options.cssOutputFilename), css = _b.css, cssMap = _b.cssMap;
        return {
            ast: this.ast,
    ...
    Generator.prototype.getUniqueNameMaker = function (params) {
        ...
        return function (name) {
            ...
            var alias = name;
            for (var i = 1; reservedNames.has(alias) ||
                _this.importedNames.has(alias) ||
                _this.usedNames.has(alias) ||
    ...
    Generator.prototype.parseJs = function () {
        var _this = this;
        ...
        if (js) {
            ...
            for (var i = 0; i < body.length; i += 1) {
                ...
                if (node.type === 'ImportDeclaration') {
                    ...
                    node.specifiers.forEach(function (specifier) {
                        _this.importedNames.add(specifier.local.name);
                    });
                }
            }
            ...
            if (templateProperties.computed) {
                ...
                var visit_1 = function visit(key) {
            ...
            if (templateProperties.components) {
                var hasNonImportedComponent_1 = false;
                templateProperties.components.value.properties.forEach(function (property) {
                    ...
                    if (_this.importedNames.has(value)) {
                        _this.importedComponents.set(key, value);
                    }
                    else {
                        hasNonImportedComponent_1 = true;
                    }
                });
                if (hasNonImportedComponent_1) {
                    // remove the specific components that were imported, as we'll refer to them directly
                    Array.from(this.importedComponents.keys()).forEach(function (key) {
                        removeObjectKey(_this.code, templateProperties.components.value, key);
                    });
                }
                else {
            ...
            if (templateProperties.onrender) {
                var key = templateProperties.onrender.key;
            ...
            if (templateProperties.onteardown) {
                var key = templateProperties.onteardown.key;
            ...
            var hasDefaultExport = !!defaultExport;
...
var Block$2 = (function () {
    ...
    Block.prototype.toString = function () {
        ...
        if (this.builders.create.isEmpty()) {
            ...
            properties.addBlock((_a = ["\n\t\t\t\tcreate: function () {\n\t\t\t\t\t", "\n\t\t\t\t\t", "\n\t\t\t\t},\n\t\t\t"], _a.raw = ["\n\t\t\t\tcreate: function () {\n\t\t\t\t\t", "\n\t\t\t\t\t", "\n\t\t\t\t},\n\t\t\t"], deindent(_a, this.builders.create, !this.builders.hydrate.isEmpty() && "this.hydrate();")));
        }
        if (this.generator.hydratable) {
            if (this.builders.claim.isEmpty()) {
                ...
                properties.addBlock((_b = ["\n\t\t\t\t\tclaim: function ( nodes ) {\n\t\t\t\t\t\t", "\n\t\t\t\t\t\t", "\n\t\t\t\t\t},\n\t\t\t\t"], _b.raw = ["\n\t\t\t\t\tclaim: function ( nodes ) {\n\t\t\t\t\t\t", "\n\t\t\t\t\t\t", "\n\t\t\t\t\t},\n\t\t\t\t"], deindent(_b, this.builders.claim, !this.builders.hydrate.isEmpty() && "this.hydrate();")));
            }
        }
        ...
        return (_m = ["\n\t\t\tfunction ", " ( ", ", #component", " ) {\n\t\t\t\t", "\n\n\t\t\t\t", "\n\n\t\t\t\treturn {\n\t\t\t\t\t", "\n\t\t\t\t};\n\t\t\t}\n\t\t"], _m.raw = ["\n\t\t\tfunction ", " ( ", ", #component",
            " ) {\n\t\t\t\t",
            "\n\n\t\t\t\t", "\n\n\t\t\t\treturn {\n\t\t\t\t\t", "\n\t\t\t\t};\n\t\t\t}\n\t\t"], deindent(_m, this.name, this.params.join(', '), this.key
            ? ", " + localKey
...
function getChildState(parent, child) {
    ...
    return assign$1({}, parent, { name: null, parentNode: null, parentNodes: 'nodes' }, child || {});
}
...
var preprocessors = {
    ...
    RawMustacheTag: function (generator, block, state, node, elementStack, componentStack, stripWhitespace) {
        var dependencies = block.findDependencies(node.expression);
        block.addDependencies(dependencies);
        var basename = block.getUniqueName('raw');
        var name = block.getUniqueName(basename + "_before");
        node._state = getChildState(state, { basename: basename, name: name });
    },
    Text: function (generator, block, state, node, elementStack, componentStack, stripWhitespace) {
        ...
        if (!/\S/.test(node.data)) {
            if (state.namespace)
                return;
            if (elementsWithoutText.has(state.parentNodeName))
                return;
        }
    ...
    EachBlock: function (generator, block, state, node, inEachBlock, elementStack, componentStack, stripWhitespace, nextSibling) {
        ...
        var listName = block.getUniqueName("each_block_value");
    ...
    Element: function (generator, block, state, node, inEachBlock, elementStack, componentStack, stripWhitespace, nextSibling) {
        ...
        if (isComponent) {
            var name = block.getUniqueName((node.name === ':Self' ? generator.name : node.name).toLowerCase());
            node._state = getChildState(state, {
                name: name,
            ...
            var name = block.getUniqueName(node.name === 'slot' ?
        ...
        if (node.children.length) {
            if (isComponent) {
                var name = block.getUniqueName((node.name === ':Self' ? generator.name : node.name).toLowerCase());
                if (node.children)
                    node._slots = new Set(['default']); // TODO only include default if there are unslotted children
...
var DomGenerator = (function (_super) {
    __extends(DomGenerator, _super);
    function DomGenerator(parsed, source, name, stylesheet, options) {
        var _this = _super.call(this, parsed, source, name, stylesheet, options) || this;
        _this.blocks = [];
...
function dom(parsed, source, stylesheet, options) {
    ...
    var computations = generator.computations, hasJs = generator.hasJs, name = generator.name, templateProperties = generator.templateProperties, namespace = generator.namespace;
    ...
    if (computations.length) {
        computations.forEach(function (_a) {
            ...
            var condition = "isInitial || " + deps.map(function (dep) { return "changed." + dep; }).join(' || ');
            var statement = "if ( @differs( ( state." + key + " = @template.computed." + key + "( " + deps
    ...
    if (generator.stylesheet.hasStyles && options.css !== false) {
        var _c = generator.stylesheet.render(options.filename), css = _c.css, cssMap = _c.cssMap;
        var textContent = stringify(options.dev ?
            css + "\n/*# sourceMappingURL=" + cssMap.toUrl() + " */" :
            css, { onlyEscapeAtSymbol: true });
        builder.addBlock((_d = ["\n\t\t\tfunction @add_css () {\n\t\t\t\tvar style = @createElement( 'style' );\n\t\t\t\tstyle.id = '", "-style';\n\t\t\t\tstyle.textContent = ", ";\n\t\t\t\t@appendNode( style, document.head );\n\t\t\t}\n\t\t"], _d.raw = ["\n\t\t\tfunction @add_css () {\n\t\t\t\tvar style = @createElement( 'style' );\n\t\t\t\tstyle.id = '", "-style';\n\t\t\t\tstyle.textContent = ", ";\n\t\t\t\t@appendNode( style, document.head );\n\t\t\t}\n\t\t"], deindent(_d, generator.stylesheet.id, textContent)));
    }
    ...
    var proto = sharedPath
        ? "@proto "
        : (_e = ["\n\t\t{\n\t\t\t", "\n\t\t}"], _e.raw = ["\n\t\t{\n\t\t\t",
            "\n\t\t}"], deindent(_e, ['destroy', 'get', 'fire', 'observe', 'on', 'set', '_set', 'teardown']
    ...
    builder.addBlock((_f = ["\n\t\tfunction ", " ( options ) {\n\t\t\toptions = options || {};\n\t\t\t", "\n\t\t\t", "\n\t\t\tthis._state = ", ";\n\t\t\t", "\n\t\t\t", "\n\t\t\t", "\n\t\t\t", "\n\n\t\t\tthis._observers = {\n\t\t\t\tpre: Object.create( null ),\n\t\t\t\tpost: Object.create( null )\n\t\t\t};\n\n\t\t\tthis._handlers = Object.create( null );\n\t\t\t", "\n\n\t\t\tthis._root = options._root || this;\n\t\t\tthis._yield = options._yield;\n\t\t\tthis._bind = options._bind;\n\t\t\tthis._slotted = options.slots || {};\n\n\t\t\t", "\n\n\t\t\t", "\n\n\t\t\t", "\n\n\t\t\t", "\n\n\t\t\tthis._fragment = @create_main_fragment( this._state, this );\n\n\t\t\tif ( options.target ) {\n\t\t\t\t", "\n\t\t\t\tthis._fragment.", "( options.target, null );\n\t\t\t}\n\n\t\t\t", "\n\t\t}\n\n\t\t@assign( ", ", ", ");\n\n\t\t", "\n\n\t\t", "\n\n\t\t", "\n\t"], _f.raw = ["\n\t\tfunction ", " ( options ) {\n\t\t\toptions = options || {};\n\t\t\t",
        ...
        "if ( !options.target && !options._root ) throw new Error( \"'target' is a required option\" );", generator.usesRefs && "this.refs = {};", templateProperties.data
        ? "@assign( @template.data(), options.data )"
        : "options.data || {}", generator.metaBindings, computations.length && "this._recompute( {}, this._state, {}, true );", options.dev &&
        Array.from(generator.expectedProperties).map(function (prop) {
            return "if ( !( '" + prop + "' in this._state ) ) console.warn( \"Component was created without expected data property '" + prop + "'\" );";
        }), generator.bindingGroups.length &&
        "this._bindingGroups = [ " + Array(generator.bindingGroups.length)
            .fill('[]')
            .join(', ') + " ];", templateProperties.ondestroy && "this._handlers.destroy = [@template.ondestroy]", generator.stylesheet.hasStyles &&
        options.css !== false &&
        "if ( !document.getElementById( '" + generator.stylesheet.id + "-style' ) ) @add_css();", templateProperties.oncreate && "var oncreate = @template.oncreate.bind( this );", (templateProperties.oncreate || generator.hasComponents || generator.hasComplexBindings || generator.hasIntroTransitions) && (_g = ["\n\t\t\t\tif ( !options._root ) {\n\t\t\t\t\tthis._oncreate = [", "];\n\t\t\t\t\t", "\n\t\t\t\t\t", "\n\t\t\t\t} ", "\n\t\t\t"], _g.raw = ["\n\t\t\t\tif ( !options._root ) {\n\t\t\t\t\tthis._oncreate = [", "];\n\t\t\t\t\t", "\n\t\t\t\t\t", "\n\t\t\t\t} ",
        "\n\t\t\t"], deindent(_g, templateProperties.oncreate && "oncreate", (generator.hasComponents || generator.hasComplexBindings) && "this._beforecreate = [];", (generator.hasComponents || generator.hasIntroTransitions) && "this._aftercreate = [];", templateProperties.oncreate && (_h = ["\n\t\t\t\t\telse {\n\t\t\t\t\t\tthis._root._oncreate.push(oncreate);\n\t\t\t\t\t}\n\t\t\t\t"], _h.raw = ["\n\t\t\t\t\telse {\n\t\t\t\t\t\tthis._root._oncreate.push(oncreate);\n\t\t\t\t\t}\n\t\t\t\t"], deindent(_h)))), generator.slots.size && "this.slots = {};", generator.hydratable
        ? (_j = ["\n\t\t\t\t\t\tvar nodes = @children( options.target );\n\t\t\t\t\t\toptions.hydrate ? this._fragment.claim( nodes ) : this._fragment.create();\n\t\t\t\t\t\tnodes.forEach( @detachNode );\n\t\t\t\t\t"], _j.raw = ["\n\t\t\t\t\t\tvar nodes = @children( options.target );\n\t\t\t\t\t\toptions.hydrate ? this._fragment.claim( nodes ) : this._fragment.create();\n\t\t\t\t\t\tnodes.forEach( @detachNode );\n\t\t\t\t\t"], deindent(_j)) : (_k = ["\n\t\t\t\t\t\t", "\n\t\t\t\t\t\tthis._fragment.create();\n\t\t\t\t\t"], _k.raw = ["\n\t\t\t\t\t\t", "\n\t\t\t\t\t\tthis._fragment.create();\n\t\t\t\t\t"], deindent(_k, options.dev && "if ( options.hydrate ) throw new Error( 'options.hydrate only works if the component was compiled with the `hydratable: true` option' );")), block.hasIntroMethod ? 'intro' : 'mount', (generator.hasComponents || generator.hasComplexBindings || templateProperties.oncreate || generator.hasIntroTransitions) && (_l = ["\n\t\t\t\tif ( !options._root ) {\n\t\t\t\t\t", "\n\t\t\t\t\t", "\n\t\t\t\t\t", "\n\t\t\t\t\t", "\n\t\t\t\t\t", "\n\t\t\t\t}\n\t\t\t"], _l.raw = ["\n\t\t\t\tif ( !options._root ) {\n\t\t\t\t\t", "\n\t\t\t\t\t", "\n\t\t\t\t\t", "\n\t\t\t\t\t", "\n\t\t\t\t\t", "\n\t\t\t\t}\n\t\t\t"], deindent(_l, generator.hasComponents && "this._lock = true;", (generator.hasComponents || generator.hasComplexBindings) && "@callAll(this._beforecreate);", (generator.hasComponents || templateProperties.oncreate) && "@callAll(this._oncreate);", (generator.hasComponents || generator.hasIntroTransitions) && "@callAll(this._aftercreate);", generator.hasComponents && "this._lock = false;")), prototypeBase, proto, options.dev && (_m = ["\n\t\t\t", ".prototype._checkReadOnly = function _checkReadOnly ( newState ) {\n\t\t\t\t", "\n\t\t\t};\n\t\t"], _m.raw = ["\n\t\t\t", ".prototype._checkReadOnly = function _checkReadOnly ( newState ) {\n\t\t\t\t",
        "\n\t\t\t};\n\t\t"], deindent(_m, name, Array.from(generator.readonly).map(function (prop) {
        return "if ( '" + prop + "' in newState && !this._updatingReadonlyProperty ) throw new Error( \"Cannot set read-only property '" + prop + "'\" );";
    }))), computations.length ? (_o = ["\n\t\t\t", ".prototype._recompute = function _recompute ( changed, state, oldState, isInitial ) {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], _o.raw = ["\n\t\t\t", ".prototype._recompute = function _recompute ( changed, state, oldState, isInitial ) {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], deindent(_o, name, computationBuilder)) : (!sharedPath && name + ".prototype._recompute = @noop;"), templateProperties.setup && "@template.setup( " + name + " );")));
    var usedHelpers = new Set();
    var result = builder
        .toString()
        .replace(/(@+)(\w*)/g, function (match, sigil, name) {
        if (sigil !== '@')
    ...
    if (sharedPath) {
        ...
        if (format === 'es') {
            var names = used.map(function (name) {
            ...
            var SHARED_1 = '__shared';
            var requires_1 = "var " + SHARED_1 + " = require( " + JSON.stringify(sharedPath) + " );";
            used.forEach(function (name) {
                var alias = generator.alias(name);
                requires_1 += "\nvar " + alias + " = " + SHARED_1 + "." + name + ";";
            });
            result = requires_1 + "\n\n" + result;
        }
        else {
...
function visitComponent$1(generator, block, node) {
    ...
    if (node.children.length) {
        ...
        generator.elementDepth -= 1;
        var slotted = Object.keys(generator.appendTargets)
            .map(function (name) { return name + ": () => `" + generator.appendTargets[name] + "`"; })
            .join(', ');
...
function stringifyAttributeValue(block, chunks) {
    return chunks
        .map(function (chunk) {
        if (chunk.type === 'Text') {
...
var SsrGenerator = (function (_super) {
    __extends(SsrGenerator, _super);
    function SsrGenerator(parsed, source, name, stylesheet, options) {
        var _this = _super.call(this, parsed, source, name, stylesheet, options) || this;
        _this.bindings = [];
        _this.renderCode = '';
        _this.elementDepth = 0;
        _this.appendTargets = null;
        // in an SSR context, we don't need to include events, methods, oncreate or ondestroy
        var _a = _this, templateProperties = _a.templateProperties, defaultExport = _a.defaultExport;
        preprocess$1(_this, parsed.html);
        _this.stylesheet.warnOnUnusedSelectors(options.onwarn);
        if (templateProperties.oncreate)
            removeNode(_this.code, defaultExport.declaration, templateProperties.oncreate);
        if (templateProperties.ondestroy)
            removeNode(_this.code, defaultExport.declaration, templateProperties.ondestroy);
        if (templateProperties.methods)
            removeNode(_this.code, defaultExport.declaration, templateProperties.methods);
        if (templateProperties.events)
            removeNode(_this.code, defaultExport.declaration, templateProperties.events);
        return _this;
    }
    ...
    SsrGenerator.prototype.removeAppendTarget = function () {
        this.appendTarget = this.appendTargets = null;
    };
    SsrGenerator.prototype.setAppendTarget = function (name) {
...
function ssr(parsed, source, stylesheet, options) {
    ...
    var computations = generator.computations, name = generator.name, hasJs = generator.hasJs, templateProperties = generator.templateProperties;
    ...
    var _a = generator.stylesheet.render(options.filename), css = _a.css, cssMap = _a.cssMap;
    var result = (_b = ["\n\t\t", "\n\n\t\tvar ", " = {};\n\n\t\t", ".filename = ", ";\n\n\t\t", ".data = function () {\n\t\t\treturn ", ";\n\t\t};\n\n\t\t", ".render = function ( state, options ) {\n\t\t\t", "\n\n\t\t\t", "\n\n\t\t\t", "\n\n\t\t\treturn `", "`.trim();\n\t\t};\n\n\t\t", ".renderCss = function () {\n\t\t\tvar components = [];\n\n\t\t\t", "\n\n\t\t\t", "\n\n\t\t\treturn {\n\t\t\t\tcss: components.map( x => x.css ).join( '\\n' ),\n\t\t\t\tmap: null,\n\t\t\t\tcomponents\n\t\t\t};\n\t\t};\n\n\t\tvar escaped = {\n\t\t\t'\"': '&quot;',\n\t\t\t\"'\": '&##39;',\n\t\t\t'&': '&amp;',\n\t\t\t'<': '&lt;',\n\t\t\t'>': '&gt;'\n\t\t};\n\n\t\tfunction __escape ( html ) {\n\t\t\treturn String( html ).replace( /[\"'&<>]/g, match => escaped[ match ] );\n\t\t}\n\t"], _b.raw = ["\n\t\t", "\n\n\t\tvar ", " = {};\n\n\t\t", ".filename = ", ";\n\n\t\t", ".data = function () {\n\t\t\treturn ", ";\n\t\t};\n\n\t\t", ".render = function ( state, options ) {\n\t\t\t",
        "\n\n\t\t\t",
        "\n\n\t\t\t",
        "\n\n\t\t\treturn \\`", "\\`.trim();\n\t\t};\n\n\t\t", ".renderCss = function () {\n\t\t\tvar components = [];\n\n\t\t\t",
        "\n\n\t\t\t",
        "\n\n\t\t\treturn {\n\t\t\t\tcss: components.map( x => x.css ).join( '\\\\n' ),\n\t\t\t\tmap: null,\n\t\t\t\tcomponents\n\t\t\t};\n\t\t};\n\n\t\tvar escaped = {\n\t\t\t'\"': '&quot;',\n\t\t\t\"'\": '&##39;',\n\t\t\t'&': '&amp;',\n\t\t\t'<': '&lt;',\n\t\t\t'>': '&gt;'\n\t\t};\n\n\t\tfunction __escape ( html ) {\n\t\t\treturn String( html ).replace( /[\"'&<>]/g, match => escaped[ match ] );\n\t\t}\n\t"], deindent(_b, hasJs && "[\u2702" + parsed.js.content.start + "-" + parsed.js.content.end + "\u2702]", name, name, stringify(options.filename), name, templateProperties.data ? "@template.data()" : "{}", name, templateProperties.data
        ? "state = Object.assign( @template.data(), state || {} );"
        : "state = state || {};", computations.map(function (_a) {
        var key = _a.key, deps = _a.deps;
        return "state." + key + " = @template.computed." + key + "( " + deps
            .map(function (dep) { return "state." + dep; })
            .join(', ') + " );";
        ...
        var expression = generator.importedComponents.get(name) ||
...
function applySelector(blocks, node, stack, toEncapsulate) {
    ...
    var _loop_1 = function () {
        ...
        if (selector.type === 'ClassSelector') {
            ...
            if (!attributeMatches(node, selector.name.name, selector.value && unquote(selector.value.value), selector.operator, selector.flags))
                return { value: false };
        }
...
var Declaration$2 = (function () {
    ...
    Declaration.prototype.minify = function (code) {
        ...
        var first = this.node.value.children[0];
        if (first.start - c > 1) {
...
var Stylesheet = (function () {
    ...
    Stylesheet.prototype.render = function (cssOutputFilename) {
        var _this = this;
        if (!this.hasStyles) {
